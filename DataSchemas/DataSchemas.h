...
SchemaBegin(name, hint) - start a schema with the specified name
SchemaEnd() - end the current schema definition

Field(type, name, default, hint) - define a simple (POD) field in the schema.

Field_Schema(type, name, default, hint) - define a field in the schema that is of a type defined by 
	another schema.  If you give a string for default it will try to load it as a string.  If you
	give a null it won't try to load a default.  Note the default only works for xml loaded objects.

Field_Schema_Array(type, name, hint) - define a field in the schema that is an array of schema types.
