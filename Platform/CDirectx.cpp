 ...
 bool CDirectX::Init ()
{
	...
	// craete the window, such that the client size is the width and height requested
	RECT wr = {0, 0, m_width, m_height};
	AdjustWindowRect(&wr, settings.m_FullScreen ? WS_POPUP : WS_OVERLAPPEDWINDOW, FALSE);
    m_hWnd = CreateWindow( m_wc.lpszClassName, "FPS",
							settings.m_FullScreen ? WS_POPUP : WS_OVERLAPPEDWINDOW, 0, 0, wr.right - wr.left, wr.bottom - wr.top,
...
void CreateBMPFile(HWND hwnd, const char *pszFile, PBITMAPINFO pbi, 
...
{
#if 1
	HDC hdc = GetDC(m_hWnd); // get the desktop device context
	...
	int height = br.y - tl.y;//GetSystemMetrics(SM_CYVIRTUALSCREEN);
	int width = br.x - tl.x;//GetSystemMetrics(SM_CXVIRTUALSCREEN);
	...
	auto a = CreateBitmapInfoStruct(m_hWnd, hbDesktop);
	CreateBMPFile(m_hWnd, fileName, a, hbDesktop, hdc);
